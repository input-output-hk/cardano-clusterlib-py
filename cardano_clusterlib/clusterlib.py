...
class ClusterLib:
    ...
    def __init__(
        ...
        self.min_change_value = 0  # TODO: proper calculation based on `minUTxOValue` needed
    ...
    def _collect_utxos_amount(self, utxos: List[UTXOData], amount: int) -> List[UTXOData]:
        ...
        collected_amount = 0
        amount_plus_change = amount + self.min_change_value
        for utxo in utxos:
            # if we were able to collect exact amount, no change is needed
            if collected_amount == amount:
                break
            # make sure the change is higher than `min_change_value`
